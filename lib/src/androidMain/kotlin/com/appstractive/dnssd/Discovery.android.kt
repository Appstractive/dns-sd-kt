...
actual fun discoverServices(type: String): Flow<DiscoveryEvent> = callbackFlow {
    ...
    fun resolveService(serviceInfo: NsdServiceInfo) {
        if (VERSION.SDK_INT >= VERSION_CODES.UPSIDE_DOWN_CAKE) {
            ...
            launch {
                try {
                    val result = nsdManager.resolveService(serviceInfo)
                ...
                } catch (ex: ResolveFailedException) {
                    Log.e(LOG_TAG, "onResolveFailed", ex)
                }
            }
        }
    }
...
private val canUseResolveExecutor = VERSION.SDK_INT >= VERSION_CODES.R &&
        SdkExtensions.getExtensionVersion(VERSION_CODES.TIRAMISU) >= 3
private suspend fun NsdManager.resolveService(serviceInfo: NsdServiceInfo): DiscoveredService {
    return resolveMutex.withLock {
        val dispatcher = currentCoroutineContext()[CoroutineDispatcher]

        suspendCancellableCoroutine<DiscoveredService> { cont ->
            val listener = object : NsdManager.ResolveListener {
                override fun onResolveFailed(serviceInfo: NsdServiceInfo, errorCode: Int) {
                    cont.resumeWithException(ResolveFailedException(errorCode))
                }
            ...
            if(canUseResolveExecutor && dispatcher != null) {
                resolveService(
                    serviceInfo,
...
private class ResolveFailedException(errorCode: Int) :
    RuntimeException("onResolveFailed: $errorCode")
